...
This document outlines the path from where we are to where we need to
be.
...
1.  `vfold_cv()` on spatial data feels as wrong as
    [`lm()`](https://rdrr.io/r/stats/lm.html) on repeated measures
