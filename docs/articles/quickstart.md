...
Features with \|correlation\| \> 0.99 are flagged as hard violations
(likely derived from the target). Features with correlation 0.95-0.99
produce warnings (may be legitimate strong predictors, but worth
checking).

## Spatial Validation

For spatial data, check that train and test regions are properly
separated:
...
BORG checks two things: 1. Whether test points are very close to
training points (\< 1% of spatial spread) 2. Whether test points fall
inside the training region’s convex hull
