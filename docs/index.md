...
BORG detects data leakage and invalid cross-validation setups before you
compute performance metrics. It checks for information reuse between
training and test data, and blocks evaluation when problems are found.
...
production. The usual cause: data leakage. Information from the test set
contaminated training, and the reported metrics were wrong.

A [Princeton meta-analysis](https://reproducible.cs.princeton.edu/)
found leakage errors in **648 published papers across 30 fields**. In
civil war prediction research, correcting leakage revealed that “complex
ML models do not perform substantively better than decades-old Logistic
Regression.” The reported gains were artifacts.
...
- **[`borg()`](https://gillescolling.com/BORG/reference/borg.md)**: Main
...
- **[`borg_inspect()`](https://gillescolling.com/BORG/reference/borg_inspect.md)**:
  Detailed inspection of specific objects
  - Works with
...
- **[`borg_diagnose()`](https://gillescolling.com/BORG/reference/borg_diagnose.md)**:
  Analyze data for dependency structure
...
**Hard Violations:** - `index_overlap` - Same row in train and test -
...
`group_leakage` - Same group in train and test - `temporal_leak` - Test
data predates training

**Soft Inflation:** - `proxy_leakage` - Feature with \|r\| 0.95-0.99
with target - `spatial_proximity` - Test points close to training -
...
| [`borg_inspect()`](https://gillescolling.com/BORG/reference/borg_inspect.md) | Detailed inspection of objects |
| [`borg_diagnose()`](https://gillescolling.com/BORG/reference/borg_diagnose.md) | Analyze data dependencies |
...
- [Risk
  Taxonomy](https://gcol33.github.io/BORG/articles/risk-taxonomy.html)
- [Framework
  Integration](https://gcol33.github.io/BORG/articles/frameworks.html)
